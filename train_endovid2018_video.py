...
from dataset.endovis2018 import ToTensor
from builders.endovis2018_builder import load_data
...
from gp_utils.process_mask import get_bounding_box, generate_input_bbox_endvis
...
def eval_sam_model(valLoader, save_eval_path, checkpoint, epoch_num=1, data_type="val", apt_flag="mlp", insert_pos="imgenc"):
    ...
    for data_four in valLoader:
        ...
        curMR, curGT = data_four["curMR"], data_four["curGT"]
...
def adjust_learning_rate_apt_img_msk(medsam2, lr_init=1e-6):
    ...
    optimizer = torch.optim.AdamW([
        ...
        # {"params": adapter_params_img, "lr": lr_init * 10},
        # {"params": medsam2.model.sam_prompt_encoder.parameters(), "lr": lr_init * 10},
...
max_epoch = 30
apt_flag = "ori"
save_type = "endvio2018_img_adp_" + apt_flag +"_1e"+str(lr)
insert_pos="imgenc"
#############################
# initialized medsam2
root_path = "/home/gxu/proj1/lesionSeg/utswtumor_track"
medsam2 = SAM2VideoTrainer(model_cfg, checkpoint, device = device)
## inject adapters
# add_adapters_to_two_way_attention_blocks(medsam2.model.sam_mask_decoder.transformer, reduction=16, apt_flag=apt_flag)
add_adapters_to_imgenc(medsam2, adapter_dim=64, num_blocks=6, apt_flag=apt_flag)
print(medsam2.model)
medsam2 = medsam2.to(device)

optimizer = adjust_learning_rate_apt_img_msk(medsam2, lr_init=lr)

scheduler = StepLR(optimizer, step_size=15, gamma=0.5) ## update learning rate
...
for epoch in range(0, max_epoch):
    ...
    for data_four in trainLoader:
        cnt_case = cnt_case + 1
        print(f"cnt case {cnt_case}")
        curMR, curGT_ori = data_four["curMR"], data_four["curGT"]
        caseID = data_four["caseID"][0]
        curMR = torch.permute(curMR, (0,2,3,4,1))
        curGT_ori = torch.permute(curGT_ori, (0,2,3,4,1))
        curMR = curMR.to(device) # BCHWD
        curGT_ori = curGT_ori.to(device) # B7HWD
        ori_H, ori_W, ori_D = curMR.shape[2], curMR.shape[3], curMR.shape[4]
        if curMR.shape[2] != image_size:
            ...
            curGT_ori = F.interpolate(curGT_ori.float(), size=(image_size, image_size, curMR.shape[4]), mode='nearest', align_corners=None)
        ...
        ## for each GT has 7 class, curGT: B7HWD
        for cls_num in range(0, curGT_ori.shape[1]):
            curGT = curGT_ori[:, cls_num:cls_num+1, ...] # B1HWD
            ## extract bbox from curGT
            curGT_np = torch.permute(curGT_ori[:, cls_num, ...], dims=(0,3,1,2)).cpu().numpy()[0,...] # DHW
            if curGT_np.sum() < 200:
                continue
            D, H, W = curGT_np.shape
            segs_3D = np.zeros((D, H, W), dtype=np.uint8)
            ...
            for n in range(0, 64):
                input_chuck, gt_chuck, first_sel_bboxes = generate_input_bbox_endvis(batch_size=2, image_depth=D, mr_vol=img_resized, 
    ...
    ## do evaluation
    # dice_res = eval_sam_model(trainLoader, save_eval_path, checkpoint, epoch_num=epoch+1,  data_type="train", apt_flag=apt_flag, insert_pos=insert_pos)
    dice_res = 0
    ## save the best model
    if epoch == 0:
        best_dice = dice_res
...
dice_res = eval_sam_model(testLoader, save_eval_path, checkpoint, epoch_num=best_epoch, data_type="test", apt_flag=apt_flag, insert_pos=insert_pos)