...
class UTSWLiver(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        curGT = nib.load(os.path.join(self.abs_path, curMR_paths[3])).get_fdata()

        # curMR_obj = nib.load(os.path.join(self.abs_path, curMR_paths[0]))
        # sx, sy, sz = curMR_obj.header.get_zooms()
        # volume_vox = sx * sy * sz
        ...
        sample = {'curMR': curMR, 'preMR': preMR, 'preGT': preGT, "curGT": curGT, "caseID": caseID}
        if self.transform:
