...
class Endovis2018(Dataset):
    """ EndoVis2018 Dataset for multi-object segmentation and tracking """
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        split_file = os.path.join(self._base_dir, f'../dataset/Endovis2018/{split}_endovis2018_rel_path.txt')
        ...
        self.abs_path = os.path.join(base_dir, "../dataset/Endovis2018")
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.abs_path, sample_path[0])
        gt_path = os.path.join(self.abs_path, sample_path[1])
        
        img_data = np.load(img_path)
        gt_data = np.load(gt_path)

        video = img_data['video']  # shape [N, 3, H, W]
        mask = gt_data['video']    # shape [N, 1, H, W]

        video = video.astype(np.float32) / 255.0  # normalize uint8 to [0,1]
        mask = mask.astype(np.uint8)
        mask = torch.from_numpy(mask)
        mask = convert_mask_exclude_background(mask) # one-hot
