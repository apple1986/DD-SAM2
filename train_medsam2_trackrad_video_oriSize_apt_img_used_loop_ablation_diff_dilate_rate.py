...
def eval_sam_model(valLoader, save_eval_path, checkpoint, epoch_num=1, num_blocks=6, data_type="val", apt_flag="mlp", insert_pos="imgenc", dilation_rate=[1,3]):
    ...
    predictor = build_sam2_video_predictor_npz_apt(model_cfg, checkpoint, use_ft=True, device=device, 
                                                   apt_flag=apt_flag, insert_pos=insert_pos, num_blocks=num_blocks, dilation_rate=dilation_rate)
...
apt_flags = ["dd_adapter_other_rate", ]
dilate_rates = {1: [1, 3, 5], 2: [1, 2], 3: [1, 2, 3], 4: [1, 3, 5, 7], 5: [1, 2, 3, 4]}

root_path = "/home/gxu/proj1/lesionSeg/utswtumor_track"
num_adp = 6

# read the key and value in dilate_rates
for key, dilation_rate in dilate_rates.items():
    print(f"Key: {key}, Value: {dilation_rate}")
    apt_flag = apt_flags[0]
    print(f"lr:{lr}")      
    save_type = "ft_img_adp_"+apt_flag+"_1e5_"+str(num_adp)+"adps_diff_di_rate_"+str(key)
    insert_pos = "imgenc"
    #############################
    # initialized medsam2
    ...
    add_adapters_to_imgenc(medsam2, adapter_dim=64, num_blocks=num_adp, apt_flag=apt_flag, dilation_rate=dilation_rate)
    print(medsam2.model)
    ...
    for epoch in range(0, max_epoch):
        ...
        dice_res = eval_sam_model(valLoader, save_eval_path, checkpoint, epoch_num=epoch+1, num_blocks=num_adp, 
                                  data_type="val", apt_flag=apt_flag, insert_pos=insert_pos, dilation_rate=dilation_rate)
    ...
    dice_res = eval_sam_model(testLoader, save_eval_path, checkpoint, epoch_num=best_epoch, num_blocks=num_adp,
                              data_type="test", apt_flag=apt_flag, insert_pos=insert_pos, dilation_rate=dilation_rate)