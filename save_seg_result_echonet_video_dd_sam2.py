...
save_eval_path = os.path.join(root_path, "paper_seg_res_echonet")
model_name = "dd_sam2"
model_cfg = args.model_cfg
# checkpoint = args.sam2_checkpoint
checkpoint = "/home/gxu/proj1/lesionSeg/utswtumor_track/MedSAM2_new/checkpoints_echocard/sam2_ft/ft_img_adp_cnn_dw_di_v3_4_1e5_bs46adps_n4/medsam2_best.pth"
...
for tv_type in tv_types:
    ...
    for data_four in testLoader:
        ...
        if curVid.shape[2] != image_size:
            curVid = F.interpolate(curVid, size=(image_size, image_size, curVid.shape[4]), mode='trilinear', align_corners=False)
        ...
        with open(os.path.join(save_seg_result_path, f"{data_type}_each_dice_hd95_nsd_asd0.txt"), "a") as f:
        ...
        if args.visualize:
            ...
            # curVid = F.interpolate(curVid, size=(ori_H, ori_W, curVid.shape[4]), mode='trilinear', align_corners=False)
            # curVid_cuda = torch.permute(curVid, dims=(0,1,4,2,3)) # BCHWD -> BCDHW

            # curVid = curVid_cuda.cpu().numpy()[0,0,...] # DHW
            # curVid = rotate_to_original_orientation(curVid) # rotate to original orientation
            # curVid_np = sitk.GetImageFromArray(curVid)
            # curVid_np.SetSpacing((1.0, 1.0, 1.0))
            # curVid_np_path = join(save_seg_result_path, f"{caseID}_mr.nii.gz")
            # sitk.WriteImage(curVid_np, curVid_np_path)
            # print(f"Save original image to {curVid_np_path}")
    ...
    ## save format results
    with open(os.path.join(save_seg_result_path, f"format_{data_type}_mean_dice_hd_nsd_asd0.txt"), "a") as f:
