...
def eval_sam_model(valLoader, save_eval_path, checkpoint, epoch_num=1, num_blocks=6, data_type="val", apt_flag="mlp", insert_pos="imgenc"):
    ...
    for data_four in valLoader:
        ...
        img_resized = torch.permute(curVid, dims=(0,4,1,2,3))[0,...] ## d3hw
...
lr = 1e-5
max_epoch = 10
# apt_flag = "cnn_dw_di_v3" # cnn_medsam2d

# apt_flags = ["mlp_medsamapt", "cnn_sammed2d_v2", "cnn_ablation_v1", "cnn_ablation_v2", 
#              "cnn_dw_di_v3_1", "cnn_dw_di_v3_2", "cnn_dw_di_v3_3", "cnn_dw_di_att"]
# apt_flags = ["cnn_dw_di_v3_4", "cnn_dw_di_v3", "mlp_medsamapt", "cnn_sammed2d_v2", "cnn_ablation_v1", "cnn_ablation_v2", "lora"]
apt_flags = ["ori"]
...
for apt_flag in apt_flags:
    print(f"lr:{lr}")      
    save_type = "ft_img_adp_"+apt_flag+"_1e5_"+str(num_adp)+"adps_echocard"
    insert_pos = "imgenc"
    #############################
    # initialized medsam2
    ...
    dice_res = eval_sam_model(testLoader, save_eval_path, checkpoint, epoch_num=0, num_blocks=num_adp,
                              data_type="test", apt_flag=apt_flag, insert_pos="XXX")