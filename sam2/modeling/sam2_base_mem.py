...
def prune_memory_features_time_split(maskmem_features, maskmem_pos_enc, total_prune_frame_num=2, prune_sim="cosine"):
    ...
    prune_frame_num = total_prune_frame_num // 2
...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        sim_method="cosine",  # ap: similarity method for memory attention
        # mem_split_ls=False
    ):
        super().__init__()
        ## ap: similarity method for memory attention
        self.sim_method = sim_method
        # self.mem_split_ls = mem_split_ls
        print(f"Using {self.sim_method} similarity for memory attention")
    ...
    def _prepare_memory_conditioned_features(
        ...
        if self.num_maskmem == 0:  # Disable memory and skip fusion
            ...
            return pix_feat # image mode

        num_obj_ptr_tokens = 0 # ap: number of object pointer tokens to use in the encoder
        tpos_sign_mul = -1 if track_in_reverse else 1
        # Step 1: condition the visual features of the current frame on previous memories ap: important
        if not is_init_cond_frame:
            ...
            cond_outputs = output_dict["cond_frame_outputs"] # dict_keys(['maskmem_features', 'maskmem_pos_enc', 'pred_masks', 'obj_ptr', 'object_score_logits'])
            ...
            for t_pos, prev in t_pos_and_prevs:
                ...
                feats = prev["maskmem_features"].to(device, non_blocking=True) # BCHW
            ...
            # if not self.training and (len(to_cat_memory) + self.num_frame_to_prune) > self.num_maskmem: # ori
            if not self.training and (len(to_cat_memory)) >= self.num_maskmem:
                ...
                this_num_frame_to_prune = 2 # ap: number of frames to prune
                ...
                # candidate_frames = torch.stack(to_cat_memory[1:-1], dim=0)  # [num_frames-2, 1024, bs, 64] # NUMxLxBxC

                ## ap: prune memory features based on the similarity method or not
                ...
                #     to_cat_memory, to_cat_memory_pos_embed = memory_compressor(to_cat_memory, to_cat_memory_pos_embed)
                # else: # prunning based on similarity
                to_cat_memory, to_cat_memory_pos_embed = prune_memory_features_time_split(to_cat_memory, to_cat_memory_pos_embed, 
                                                                                          total_prune_frame_num=this_num_frame_to_prune, prune_sim=self.sim_method)
                ...
                # elif self.sim_method == "cosine_unsim":
    ...
    def _track_step(
        ...
        current_out = {"point_inputs": point_inputs, "mask_inputs": mask_inputs} # save current outputs
        # High-resolution feature maps for the SAM head, reshape (HW)BC => BCHW, Bx32x128x128,Bx64x64x64
        ...
        if mask_inputs is not None and self.use_mask_input_as_output_without_sam:
            ...
            # fused the visual feature with previous memory features in the memory bank: important: memory attention
            pix_feat = self._prepare_memory_conditioned_features(
                frame_idx=frame_idx,
                is_init_cond_frame=is_init_cond_frame,
                current_vision_feats=current_vision_feats[-1:], # 1024xBx256, LxBxC, there three different spatial resolution feats in current_vision_feats
                current_vision_pos_embeds=current_vision_pos_embeds[-1:],
                feat_sizes=feat_sizes[-1:],
                output_dict=output_dict, # include previous memory features: dict_keys(['cond_frame_outputs', 'non_cond_frame_outputs'])
