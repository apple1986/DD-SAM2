...
# def prune_memory_features_time_split(maskmem_features, maskmem_pos_enc, prune_frame_num=2, prune_sim="cosine"):

#     middle_frame_idx = len(maskmem_features) // 2 
#     # get the first and last frame's feature: save the first and the last memory frames
#     first_vision_feature = maskmem_features[0] # LBC
#     last_vision_feature = maskmem_features[-1] # LBC
#     repeat_dims_first = [middle_frame_idx-1] + [1] * (len(first_vision_feature.shape)) # exclude the first frame
#     repeat_dims_last = [len(maskmem_features) - 1 - middle_frame_idx] + [1] * (len(last_vision_feature.shape)) # exclude the last frame
    
#     last_frame_expanded = last_vision_feature.unsqueeze(0).repeat(*repeat_dims_last)  # [num_frames-2, 1024, bs, 64]
#     first_frame_expanded = first_vision_feature.unsqueeze(0).repeat(*repeat_dims_first)  # [num_frames-2, 1024, bs, 64]
#     # get the candidate frames: exluude the first and last frames
#     candidate_frames_last = torch.stack(maskmem_features[middle_frame_idx:-1], dim=0)  # [num_frames-2, 1024, bs, 64] # NUMxLxBxC   
#     candidate_frames_first = torch.stack(maskmem_features[1:middle_frame_idx], dim=0)  # [num_frames-2, 1024, bs, 64] # NUMxLxBxC         
...
#         delete_indices = []
#     if prune_sim== "cosine":
#         similarities_last = torch.cosine_similarity(candidate_frames_last, last_frame_expanded, dim=-1)  # [num_frames-2, 1024, bs]
#         similarities_last = torch.sum(similarities_last, dim=1)  # [num_frames-2, bs]
#         similarities_first = torch.cosine_similarity(candidate_frames_first, first_frame_expanded, dim=-1)  # [num_frames-2, 1024, bs]
#         similarities_first = torch.sum(similarities_first, dim=1)  # [num_frames-2, bs]

#     elif prune_sim == "cosine_unsim":
#         similarities = 1 - torch.cosine_similarity(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]   
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "euclidean":
#         similarities = euclidean_similarity(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "dot":
#         similarities = dot_product_similarity(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "pearson":
#         similarities = pearson_correlation(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "manhattan":
#         similarities = manhattan_similarity(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]                                                        
#     elif prune_sim== "ssim":
#         similarities = structural_similarity_index(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]  

#     ## spatial dimension
#     elif prune_sim== "cosine_sp":
#         similarities = cosine_similarity_sp(candidate_frames_last, last_frame_expanded, dim=-1)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "euclidean_sp":
#         similarities = euclidean_similarity_sp(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "dot_sp":
#         similarities = dot_product_similarity_sp(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "pearson_sp":
#         similarities = pearson_correlation_sp(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
#     elif prune_sim== "manhattan_sp":
#         similarities = manhattan_similarity_sp(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]                                                          
#     elif prune_sim== "ssim_sp":
#         similarities = ssim_channelwise_sp(candidate_frames_last, last_frame_expanded)  # [num_frames-2, 1024, bs]
#         similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]     
#     # else:  
#     #     delete_indices = None
...
## ap: sort whole memory features by similarity to the middle frame
# def prune_memory_features_time_split(maskmem_features, maskmem_pos_enc, prune_frame_num=2, prune_sim="cosine"):
#     if len(maskmem_features) <= 3 or prune_frame_num == 0:
#         # nothing to prune if too few frames
#         return maskmem_features, maskmem_pos_enc

#     middle_idx = len(maskmem_features) // 2

#     # First and last memory frame as references
#     first_feature = maskmem_features[0]  # LxBxC
#     last_feature = maskmem_features[-1]
#     repeat_dims_first = [middle_idx-1] + [1] * (len(first_feature.shape)) # exclude the first frame
#     repeat_dims_last = [len(maskmem_features) - 1 - middle_idx] + [1] * (len(last_feature.shape)) # exclude the last frame

#     # Split memory features into two segments (excluding first and last)
#     first_half = maskmem_features[1:middle_idx]          # before middle
#     second_half = maskmem_features[middle_idx:-1]        # after middle

#     # Expand for broadcasting
#     first_expanded = first_feature.unsqueeze(0).repeat(*repeat_dims_first)  # [N1, L, B, C]
#     last_expanded = last_feature.unsqueeze(0).repeat(*repeat_dims_last)    # [N2, L, B, C]

#     # Stack candidate features
#     first_candidates = torch.stack(first_half, dim=0)    # [N1, L, B, C]
#     second_candidates = torch.stack(second_half, dim=0)  # [N2, L, B, C]

#     def compute_similarity(a, b, mode):
#         if mode == "cosine":
#             sim = torch.cosine_similarity(a, b, dim=-1)  # [N, L, B]
#         else:
#             raise NotImplementedError(f"Similarity mode '{mode}' is not supported yet.")
#         return torch.sum(sim, dim=1).mean(dim=1)  # [N] → sum over L, mean over B

#     if prune_sim.lower() != "none":
#         sim_first = compute_similarity(first_candidates, first_expanded, prune_sim)
#         sim_second = compute_similarity(second_candidates, last_expanded, prune_sim)

#         # Combine both parts with index offsets
#         sim_scores = torch.cat([sim_first, sim_second], dim=0)  # [N_total]
#         offset_indices = list(range(1, middle_idx)) + list(range(middle_idx, len(maskmem_features) - 1))  # exclude first & last

#         # Get top-k most similar indices
#         _, sorted_indices = torch.sort(sim_scores, descending=True)
#         delete_raw_indices = [offset_indices[i.item()] for i in sorted_indices[:prune_frame_num]]
#         delete_raw_indices = sorted(delete_raw_indices, reverse=True)  # reverse to avoid index shift
...
def prune_memory_features_time_split(maskmem_features, maskmem_pos_enc, total_prune_frame_num=2, prune_sim="cosine"):
    ...
    def compute_similarity(candidates, reference, mode):
        ...
        if mode == "cosine":
            sim = torch.cosine_similarity(candidates_stacked, reference_expanded, dim=-1)  # [N, L, B]
        else:
