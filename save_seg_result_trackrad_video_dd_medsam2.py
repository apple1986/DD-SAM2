...
def infer_video_mode_one_direction(img_resized, predictor, bbox, H, W, key_slice_idx_offset, propagate_with_box):
    ...
    return segs_3D
########visualization
def visualize_features(before, after, method='pca', num_components=3):
    """
    Visualizes features before and after the adapter using PCA or t-SNE.
    """
    def reduce_features(feat, method, num_components):
        B, C, H, W = feat.shape
        feat_flat = feat.permute(0, 2, 3, 1).reshape(-1, C).cpu().numpy()
        if method == 'pca':
            reducer = PCA(n_components=num_components)
        elif method == 'tsne':
            reducer = TSNE(n_components=num_components, perplexity=30, learning_rate=200, n_iter=1000)
        else:
            raise ValueError("method must be 'pca' or 'tsne'")
        reduced = reducer.fit_transform(feat_flat)
        return reduced.reshape(B, H, W, num_components)

    def plot_reduced_feature(reduced, title):
        B, H, W, C = reduced.shape
        for i in range(min(B, 1)):  # Plot only the first sample
            img = reduced[i]
            if C == 3:
    ...
    reduced_before = reduce_features(before, method, num_components)
    reduced_after = reduce_features(after, method, num_components)

    plot_reduced_feature(reduced_before, "Before Adapter")
    plot_reduced_feature(reduced_after, "After Adapter")
    plot_comparison(reduced_before, reduced_after, method='pca')
...
def plot_comparison(reduced_before, reduced_after, method='pca'):
    B, H, W, C = reduced_before.shape
    for i in range(min(B, 1)):  # Show one sample
        fig, axes = plt.subplots(1, 2, figsize=(12, 5))

        for ax, img, title in zip(axes, 
                                  [reduced_before[i], reduced_after[i]],
                                  [f"Before Adapter ({method.upper()})", f"After Adapter ({method.upper()})"]):
            if C == 3:
                img = normalize_image(img)
                ax.imshow(img)
            else:
                ax.imshow(normalize_image(img[..., 0]), cmap='viridis')
            ax.set_title(title)
            ax.axis('off')
...
def normalize_map(map_2d):
    map_2d = map_2d - map_2d.min()
    if map_2d.max() > 0:
        map_2d = map_2d / map_2d.max()
    return map_2d

def plot_avg_max_features(features, title_prefix="Feature Map"):
    """
    features: torch.Tensor of shape (B, C, H, W)
    """
    B, C, H, W = features.shape

    for i in range(min(B, 1)):  # Visualize only the first item in the batch
        feature = features[i]
        avg_map = np.mean(feature.cpu().numpy(), axis=0)
        max_map = np.max(feature.cpu().numpy(), axis=0)

        avg_map = normalize_map(avg_map)
        max_map = normalize_map(max_map)

        fig, axes = plt.subplots(1, 2, figsize=(10, 4))
        axes[0].imshow(avg_map, cmap='viridis')
        axes[0].set_title(f"{title_prefix} - Avg Pool")
        axes[0].axis('off')

        axes[1].imshow(max_map, cmap='inferno')
        axes[1].set_title(f"{title_prefix} - Max Pool")
        axes[1].axis('off')
...
def medsam_inference(
    ...
    low_res_masks_logits, _, _, _ = medsam_model.sam2_model.sam_mask_decoder(
        ...
        multimask_output=False,
        repeat_image=False,
        high_res_features=high_res_features, # two: Bx64x128x128; Bx32x256x256
    )
...
checkpoint = "/home/gxu/proj1/lesionSeg/utswtumor_track/MedSAM2_new/checkpoints/medsam2_ft/ft_img_adp_cnn_dw_di_v3_4_1e5_n2_6adps_check_used/medsam2_best.pth"
...
predictor = build_sam2_video_predictor_npz_apt(model_cfg, checkpoint, use_ft=use_ft, apt_flag="cnn_dw_di_v3_4", insert_pos="imgenc") #
...
for tv_type in tv_types:
    ...
    save_seg_result_path = os.path.join(save_eval_path, f"seg_{tv_type}")
