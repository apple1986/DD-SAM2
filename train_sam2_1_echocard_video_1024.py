...
def eval_sam_model(valLoader, save_eval_path, checkpoint, epoch_num=1, num_blocks=6, data_type="val", apt_flag="mlp", insert_pos="imgenc"):
    ...
    for data_four in valLoader:
        ...
        curVid, curGT = data_four["curVid"], data_four["curGT"]
        caseID = data_four["caseID"][0]
        curVid = curVid.to(device) # BCHWD
        curGT = curGT.to(device) # BCHWD
        ori_H, ori_W, ori_D = curVid.shape[2], curVid.shape[3], curVid.shape[4]
        if curVid.shape[2] != IMG_SIZE:
            curVid = F.interpolate(curVid, size=(IMG_SIZE, IMG_SIZE, curVid.shape[4]), mode='trilinear', align_corners=False)
            curGT = F.interpolate(curGT.float(), size=(IMG_SIZE, IMG_SIZE, curVid.shape[4]), mode='nearest', align_corners=None)
        ...
        img_resized = torch.permute(curVid, dims=(0,4,1,2,3))[0,...] ## d1hw
...
def random_crop_depth(tensor, crop_depth=250):
    """
    Randomly crop a tensor along the depth axis if depth > crop_depth.

    Args:
        tensor (torch.Tensor): Shape [B, C, H, W, D]
        crop_depth (int): Number of slices to retain

    Returns:
        torch.Tensor: Cropped tensor of shape [B, C, H, W, crop_depth]
    """
    assert tensor.dim() == 5, "Input tensor must be 5D: [B, C, H, W, D]"
    depth = tensor.size(-1)

    if depth <= crop_depth:
        return tensor  # No cropping needed

    start = random.randint(0, depth - crop_depth)
    return tensor[..., start:start + crop_depth]

def random_crop_image_and_label(image, label, crop_depth=250):
    """
    Randomly crop image and label along the depth axis if depth > crop_depth.

    Args:
        image (torch.Tensor): Image tensor of shape [B, C, H, W, D]
        label (torch.Tensor): Label tensor of shape [B, 1, H, W, D] or same shape as image
        crop_depth (int): Number of slices to retain

    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Cropped image and label tensors
    """
    assert image.dim() == 5 and label.dim() == 5, "Both image and label must be 5D tensors"
    depth = image.size(-1)
    assert image.shape[-1] == label.shape[-1], "Image and label must have the same depth"

    if depth <= crop_depth:
        return image, label  # No cropping needed

    start = random.randint(0, depth - crop_depth)
    end = start + crop_depth
...
lr = 1e-5
max_epoch = 10
apt_flags = ["dd_adapter","mlp_medsamapt", "cnn_sammed2d_v2", "lora"]

root_path = "/home/gxu/proj1/lesionSeg/utswtumor_track"
num_adp = 6
for apt_flag in apt_flags:
    print(f"lr:{lr}")      
    save_type = "ft_img_adp_"+apt_flag+"_1e5_bs4"+str(num_adp)+"adps_n4"
    insert_pos = "imgenc"
    #############################
    # initialized medsam2
    ...
    for epoch in range(0, max_epoch):
        ...
        for data_four in trainLoader:
            ...
            curVid, curGT = data_four["curVid"], data_four["curGT"]
            caseID = data_four["caseID"][0]
            curVid, curGT = random_crop_image_and_label(curVid, curGT)

            curVid = curVid.to(device) # BCHWD
            curGT = curGT.to(device) # BCHWD
            ori_H, ori_W, ori_D = curVid.shape[2], curVid.shape[3], curVid.shape[4]
            if curVid.shape[2] != image_size:
                img_resized = F.interpolate(curVid, size=(image_size, image_size, curVid.shape[4]), mode='trilinear', align_corners=False)
                curGT = F.interpolate(curGT.float(), size=(image_size, image_size, curVid.shape[4]), mode='nearest', align_corners=None)
            ...
            bbox_dict, marker_zids = get_bounding_box(box_func, curGT_np, label_id=1, bbox_shift=args.bbox_shift)
            ## data normalization
            img_resized = torch.permute(img_resized, dims=(0,4,1,2,3))[0,...] ## d3hw
            img_mean=(0.485, 0.456, 0.406)
            img_std=(0.229, 0.224, 0.225)
            img_mean = torch.tensor(img_mean, dtype=torch.float32)[:, None, None].to(device)
            img_std = torch.tensor(img_std, dtype=torch.float32)[:, None, None].to(device)
            ...
            ## for each vol, random sample 2 times each time generate 2 batch, each batch has 8 slices
            dice_case_all = 0
            NUM_SAMPLE = 2
            for n in range(0, NUM_SAMPLE):
                input_chuck, gt_chuck, first_sel_bboxes = generate_input_bbox_for_empty_bbox(batch_size=2, image_depth=D, mr_vol=img_resized, 
                                                                            gt_vol=curGT, bbox_dict=bbox_dict, device=device, NUM_SLICE=4,
                                                                            image_size=image_size, MAX_AREA=True)
                ## empty cache
            ...
            with open(os.path.join(save_eval_path, f"training_case_loss_dice_log.txt"), "a") as f:
                f.write(f"loss: {loss.item()/NUM_SAMPLE:.6f}\tdice: {dice_case_all/NUM_SAMPLE:.4f}\n")
